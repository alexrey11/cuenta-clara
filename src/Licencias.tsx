        ...
        <div className="p-6 bg-gray-50 dark:bg-gray-900 min-h-screen">
            <div className="max-w-5xl mx-auto">
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 mb-6">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">Genera y administra los códigos de activación para tus clientes</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div className="bg-gradient-to-br from-yellow-500 to-yellow-600 text-white p-6 rounded-2xl shadow-md">
                        <p className="text-sm opacity-90 mb-2">Disponibles</p>
                        <p className="text-4xl font-bold">{disponibles.length}</p>
                    </div>
                    <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white p-6 rounded-2xl shadow-md">
                        <p className="text-sm opacity-90 mb-2">Vendidas</p>
                        <p className="text-4xl font-bold">{vendidas.length}</p>
                    </div>
                    <div className="bg-gradient-to-br from-green-500 to-green-600 text-white p-6 rounded-2xl shadow-md">
                        <p className="text-sm opacity-90 mb-2">Activadas</p>
                        <p className="text-4xl font-bold">{activadas.length}</p>
                    </div>
                </div>

                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 mb-6">
                    <h2 className="text-xl font-semibold mb-4 text-gray-800 dark:text-gray-200">Generar Nueva Licencia</h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <input
                            ...
                            className="border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
                        <select
                            value={plan}
                            ...
                            {licencias.map((licencia) => (
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        <div className={`w-3 h-3 rounded-full ${licencia.estado === 'activada' ? 'bg-green-500' :
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-600 dark:text-gray-400">
                                                Cliente: {licencia.cliente} | Plan: {licencia.plan === 'anual' ? 'Anual (5,000 CUP)' : 'Mensual (500 CUP)'}
                                            </p>
                                            <p className="text-xs text-gray-400 dark:text-gray-500">
                                                Creada: {new Date(licencia.fechaCreacion).toLocaleDateString('es-ES')}
                                                {licencia.fechaActivacion && ` | Activada: ${new Date(licencia.fechaActivacion).toLocaleDateString('es-ES')}`}
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex gap-2">
                                        <span className={`px-3 py-1 rounded-full text-xs font-semibold ${licencia.estado === 'activada' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' :
                                        ...
                                        <button
                                            ...
                                            className="bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300 px-3 py-1 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-500 transition-colors text-sm"
                                        >
                                            📋 Copiar
                                        </button>
                                        <button
                                            onClick={() => eliminarLicencia(licencia.id!)}
                                            className="bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 px-3 py-1 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/50 transition-colors text-sm"
                                        >
                                            🗑️
                                        </button>
                                    </div>
