...
const FEATURES = [
    {
        ...
        desc: "Cobra en segundos con carrito",
        tile: "from-sky-500 to-blue-600",
    },
    {
        icon: "📦",
        title: "Inventario",
        desc: "Stock y precios siempre al día",
        tile: "from-violet-500 to-purple-600",
    },
    {
        icon: "👥",
        title: "Clientes y fiados",
        desc: "Quién te debe y cuánto, claro",
        tile: "from-amber-500 to-orange-600",
    },
    {
        icon: "📊",
        title: "Reportes",
        desc: "Ganancias reales, sin calcular",
...
.cc-blob { animation: cc-blob 18s ease-in-out infinite; }
...
.cc-fade-up { opacity: 0; animation: cc-fade-up .7s cubic-bezier(.22,1,.36,1) forwards; }
...
.cc-shine { animation: cc-shine 3.5s ease-in-out infinite; }

@keyframes cc-ring {
  0%   { transform: scale(.85); opacity: .55; }
  100% { transform: scale(1.7);  opacity: 0; }
}
.cc-ring { animation: cc-ring 2.6s ease-out infinite; }
...
@media (prefers-reduced-motion: reduce) {
  .cc-blob, .cc-float, .cc-shine, .cc-ring, .cc-gradient-text { animation: none; }
    ...
    return (
        <div className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-slate-950 p-4 md:p-6">
            ...
            <div className="pointer-events-none absolute inset-0" aria-hidden="true">
                <div className="cc-blob absolute -left-24 -top-24 h-96 w-96 rounded-full bg-blue-600/40 blur-3xl" />
                <div
                    className="cc-blob absolute -right-24 top-1/3 h-[28rem] w-[28rem] rounded-full bg-indigo-600/40 blur-3xl"
                    style={{ animationDelay: "-6s" }}
                />
                <div
                    className="cc-blob absolute -bottom-32 left-1/4 h-80 w-80 rounded-full bg-emerald-500/25 blur-3xl"
            ...
            <div className="relative w-full max-w-lg">
                <div className="max-h-[95vh] overflow-y-auto rounded-3xl border border-white/60 bg-white/95 p-6 shadow-[0_30px_90px_-25px_rgba(0,0,0,0.75)] backdrop-blur-xl md:p-9">
                    ...
                    <div className="cc-fade-up mb-5 flex justify-center" style={{ animationDelay: "0.05s" }}>
                        <div className="relative">
                            <span className="cc-ring absolute inset-0 rounded-3xl bg-blue-500/40" />
                            <div className="cc-float relative flex h-20 w-20 items-center justify-center rounded-3xl bg-gradient-to-br from-blue-500 via-indigo-600 to-violet-600 text-4xl shadow-xl shadow-indigo-600/40 ring-4 ring-white/70">
                                📊
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="cc-fade-up mb-6 text-center" style={{ animationDelay: "0.1s" }}>
                        <h1 className="cc-gradient-text bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 bg-clip-text text-4xl font-black tracking-tight text-transparent md:text-5xl">
                            CuentaClara
                        </h1>
                        <p className="mt-2 text-base font-medium text-gray-600 md:text-lg">
                            Tu negocio, siempre en orden.{" "}
                            <span className="font-bold text-gray-800">Aunque no haya internet.</span>
                        </p>
                    </div>
                    ...
                    <div className="mb-6 grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                        {FEATURES.map((f, i) => (
                            <div
                                key={f.title}
                                className="cc-fade-up group flex items-center gap-3 rounded-2xl border border-gray-100 bg-gray-50/80 p-3 transition-all duration-200 hover:-translate-y-0.5 hover:border-blue-200 hover:bg-white hover:shadow-md"
                            ...
                            >
                                ...
                                <span className="min-w-0">
                                    <span className="block text-sm font-bold text-gray-800">{f.title}</span>
                                    <span className="block truncate text-xs text-gray-500">{f.desc}</span>
                                </span>
                            </div>
                        ))}
                    </div>
                    ...
                    {/* Sellos de confianza */}
                    <div
                        className="cc-fade-up mb-6 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 rounded-2xl bg-gradient-to-r from-blue-50 to-indigo-50 px-4 py-3 text-[11px] font-semibold text-gray-600 md:text-xs"
                    ...
                    <div className="cc-fade-up" style={{ animationDelay: "0.5s" }}>
                        ...
                        >
                            <span className="cc-shine pointer-events-none absolute inset-y-0 left-0 w-1/3 bg-gradient-to-r from-transparent via-white/35 to-transparent" />
                    ...
                    <div className="cc-fade-up my-5 flex items-center gap-3" style={{ animationDelay: "0.55s" }}>
                        ...
                        <span className="text-[11px] font-semibold uppercase tracking-wider text-gray-400">o</span>
                    ...
                    <button
                        ...
                        className="cc-fade-up w-full rounded-2xl border-2 border-gray-200 bg-white px-6 py-3 text-sm font-bold text-gray-700 transition-all duration-200 hover:border-blue-500 hover:bg-blue-50 hover:text-blue-700 md:text-base"
                        ...
                        <div className="flex items-center gap-2">
                            <div className="flex -space-x-2">
                                {["MJ", "CR", "LP", "AT"].map((ini, i) => (
                                    <span
                                        key={ini}
                                        className={`flex h-7 w-7 items-center justify-center rounded-full border-2 border-white text-[9px] font-bold text-white ${["bg-blue-500", "bg-emerald-500", "bg-amber-500", "bg-violet-500"][i]
                                            }`}
                                    >
                                        {ini}
                                    </span>
