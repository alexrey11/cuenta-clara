...
export default function Devoluciones({ usuarioActual }: DevolucionesProps) {
    ...
    const procesarDevolucion = async () => {
        if (!ventaSel || !cantDev || !motivo.trim()) {
        ...
        // Obtener items de la venta (soporta ambos formatos)
        ...
        if (items.length === 0) {
            alert('Venta sin productos');
            return;
        }

        // Para simplificar, devolvemos solo del primer item
        const primerItem = items[0];

        if (c <= 0 || c > primerItem.cantidad) {
            alert(`Cantidad inválida. Máximo: ${primerItem.cantidad}`);
            return;
        }
        ...
        if (prod) {
            await db.productos.update(prod.id!, { stockActual: prod.stockActual + c });
        }

        if (ventaSel.esFiado && ventaSel.clienteId) {
            const cli = await db.clientes.get(ventaSel.clienteId);
            if (cli) {
                await db.clientes.update(cli.id!, { saldoPendiente: Math.max(0, cli.saldoPendiente - monto) });
            }
        }
    ...
    const getNombreVenta = (v: Venta): string => {
        if (v.items && v.items.length > 0) {
            return v.items.map(i => i.productoNombre).join(', ');
        }
        return v.productoNombre || 'Venta';
    };

    const getCantidadVenta = (v: Venta): number => {
        if (v.items && v.items.length > 0) {
            return v.items.reduce((sum, i) => sum + i.cantidad, 0);
        }
        return v.cantidad || 1;
    };
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div className="bg-gradient-to-br from-orange-500 to-orange-600 text-white p-6 rounded-2xl shadow-md">
                        <p className="text-sm opacity-90 mb-2">Total Devoluciones</p>
                        <p className="text-4xl font-bold">{devoluciones.length}</p>
                    </div>
                    <div className="bg-gradient-to-br from-red-500 to-red-600 text-white p-6 rounded-2xl shadow-md">
                        <p className="text-sm opacity-90 mb-2">Reembolsado</p>
                        <p className="text-4xl font-bold">${totalReembolsado.toFixed(2)}</p>
                    </div>
                    <div className="bg-gradient-to-br from-blue-500 to-blue-600 text-white p-6 rounded-2xl shadow-md">
                        <p className="text-sm opacity-90 mb-2">Disponibles</p>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-4 mb-6">
                    <input
                        ...
                        className="w-full border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 mb-6">
                    ...
                    {ventasFiltradas.length === 0 ? (
                        <p className="text-gray-500 dark:text-gray-400 text-center py-8">Sin ventas</p>
                    ) : (
                            ...
                            {ventasFiltradas.slice(0, 10).map((v) => (
                                <div key={v.id} className="flex items-center justify-between bg-gray-50 dark:bg-gray-700 p-4 rounded-xl border border-gray-100 dark:border-gray-600">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h3 className="font-semibold text-gray-800 dark:text-gray-200">{getNombreVenta(v)}</h3>
                                            <p className="text-sm text-gray-500 dark:text-gray-400">
                                                {new Date(v.fecha).toLocaleDateString('es-ES')} | {v.vendedorNombre}
                                                {v.clienteNombre && ` | 👤 ${v.clienteNombre}`}
                                            </p>
                                            <p className="text-sm text-gray-600 dark:text-gray-300">
                                                Cant: {getCantidadVenta(v)} | Total: ${v.total.toFixed(2)}
                                            </p>
                                        </div>
                                    </div>
                                    <button
                                        ...
                                        className="bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 px-4 py-2 rounded-lg hover:bg-orange-200 dark:hover:bg-orange-900/50 transition-colors font-semibold"
                                    >
                                        Devolver
                                    </button>
                                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6">
                    ...
                    {devoluciones.length === 0 ? (
                        <p className="text-gray-500 dark:text-gray-400 text-center py-8">Sin devoluciones</p>
                    ) : (
                        <div className="space-y-3">
                            {devoluciones.map((d) => (
                                <div key={d.id} className="bg-gray-50 dark:bg-gray-700 p-4 rounded-xl border border-gray-100 dark:border-gray-600">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <h3 className="font-semibold text-gray-800 dark:text-gray-200">{d.productoNombre}</h3>
                                            <p className="text-sm text-gray-500 dark:text-gray-400">
                                                {new Date(d.fecha).toLocaleDateString('es-ES')} | Por: {d.realizadoPor}
                                            </p>
                                            <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">
                                                Cant: {d.cantidad} | Reembolso: ${d.montoReembolsado.toFixed(2)}
                                            </p>
                                            <p className="text-sm text-orange-600 dark:text-orange-400 mt-1 italic">
                                                Motivo: {d.motivo}
                                            </p>
                                        </div>
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                            ...
                            <div className="p-6 space-y-4">
                                <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-xl">
                                    <p className="font-semibold text-gray-800 dark:text-gray-200">{getNombreVenta(ventaSel)}</p>
                                    <p className="text-sm text-gray-500 dark:text-gray-400">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                        Cantidad (máx: {getCantidadVenta(ventaSel)})
                                    </label>
                                    <input
                                        type="number"
                                        min="1"
                                        max={getCantidadVenta(ventaSel)}
                                        value={cantDev}
                                        onChange={(e) => setCantDev(e.target.value)}
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <button
                                        ...
                                        className="flex-1 bg-orange-600 text-white py-3 rounded-lg hover:bg-orange-700 font-semibold shadow-md"
                                    >
                                        Procesar
                                    </button>
                                </div>
